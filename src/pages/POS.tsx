...
import {
  ...
  CheckCircle, AlertCircle, Package, X, Percent, Hash
} from 'lucide-react'
...
export default function POS() {
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="card p-10 text-center max-w-md">
          ...
          <p className="text-gray-500 mb-6 text-sm">
            Para realizar ventas debes abrir tu caja primero.
          </p>
          <a href="/caja" className="btn btn-primary btn-lg">
            Ir a Caja
          </a>
        </div>
      </div>
  ...
  return (
    <div className="flex gap-4 h-[calc(100vh-7rem)]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto">
          {showCombos || (search && displayedCombos.length > 0) ? (
            <>
              {search && <p className="text-xs text-gray-500 mb-2 font-medium">Combos</p>}
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-4">
                {(search ? displayedCombos : combos).map(combo => {
                  ...
                  return (
                    <button
                      ...
                      className="bg-white border-t-4 border-t-red-600 border border-gray-100 rounded-2xl overflow-hidden text-left hover:border-red-300 hover:shadow-xl transition-all duration-200 active:scale-95 group flex flex-col"
                    >
                      <div className="w-full h-56 bg-white p-3 border-b border-gray-100 flex items-center justify-center overflow-hidden shrink-0 relative">
                        <img
                          src={imgSrc}
                          alt={combo.name}
                      ...
                      <div className="p-3.5 flex-1 flex flex-col bg-white">
                        <p className="text-xs font-bold text-gray-800 line-clamp-2 leading-snug flex-1">{combo.name}</p>
                        <p className="text-lg font-extrabold text-red-600 mt-1">{formatCurrency(combo.price, 'L')}</p>
                        <div className="flex items-center justify-center mt-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl py-2 text-xs font-bold transition-all shadow-sm group-hover:shadow-md">
          ...
          {(!showCombos || search) && (
            <>
              {search && displayedProducts.length > 0 && <p className="text-xs text-gray-500 mb-2 font-medium">Productos</p>}
                ...
                {displayedProducts.map(product => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-3.5 flex-1 flex flex-col bg-white">
                        <p className="text-xs font-bold text-gray-800 line-clamp-2 leading-snug flex-1">{product.name}</p>
                        <p className="text-lg font-extrabold text-red-600 mt-1">{formatCurrency(product.price, 'L')}</p>
                        <div className="flex items-center justify-center mt-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl py-2 text-xs font-bold transition-all shadow-sm group-hover:shadow-md">
                          <Plus size={14} className="mr-1" /> Agregar
                        </div>
                      </div>
      ...
      {/* RIGHT: Cart */}
      <div className="w-80 xl:w-96 flex flex-col bg-white rounded-2xl border border-gray-100 shadow-sm shrink-0 overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between bg-red-50/50">
          <div className="flex items-center gap-2">
            ...
            {cart.length > 0 && (
              <span className="bg-red-600 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center shadow-sm">
                {cart.reduce((s, i) => s + i.quantity, 0)}
              </span>
            )}
          </div>
          {cart.length > 0 && (
            <button onClick={() => setCart([])} className="text-xs font-medium text-red-500 hover:text-red-700">
              Vaciar
            </button>
          )}
        </div>
            ...
            cart.map(item => (
              <div key={`${item.type}-${item.id}`}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-red-600 font-bold">{formatCurrency(item.price * item.quantity, 'L')}</p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    onClick={() => updateQty(item.id, item.type, -1)}
                    className="w-6 h-6 rounded-md bg-white border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-red-50 hover:border-red-200 hover:text-red-500"
                  >
                    <Minus size={10} />
                  </button>
                  ...
                  <button
                    onClick={() => updateQty(item.id, item.type, 1)}
                    className="w-6 h-6 rounded-md bg-white border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-emerald-50 hover:border-emerald-200 hover:text-emerald-500"
                  >
                    <Plus size={10} />
                  </button>
                  <button
                    onClick={() => removeFromCart(item.id, item.type)}
                    className="w-6 h-6 rounded-md bg-white border border-gray-200 flex items-center justify-center text-gray-400 hover:bg-red-50 hover:border-red-200 hover:text-red-500 ml-1"
                  >
                    <Trash2 size={10} />
                  </button>
                </div>
        ...
        <div className="px-4 py-3 border-t border-gray-100 space-y-2">
          ...
          <div className="flex justify-between text-lg font-bold text-gray-900 pt-1 border-t border-dashed border-gray-200">
            <span>TOTAL</span>
            <span className="text-red-600 font-extrabold text-xl">{formatCurrency(total, 'L')}</span>
          </div>
        </div>
      ...
      >
        <div className="space-y-4">
          {/* Order summary */}
          <div className="bg-red-50/40 border border-red-100 rounded-xl p-3 space-y-1">
            {cart.map(item => (
              <div key={`${item.type}-${item.id}`} className="flex justify-between text-sm">
                <span className="text-gray-700">{item.quantity}× {item.name}</span>
                <span className="font-semibold">{formatCurrency(item.price * item.quantity, 'L')}</span>
              </div>
            ))}
            <div className="border-t border-dashed border-red-200 pt-1 mt-1">
              {discountAmount > 0 && (
              ...
              <div className="flex justify-between font-extrabold text-red-600 text-base">
                <span>Total</span>
          ...
          {paymentMethod === 'cash' && (
            <div>
              ...
              {received >= total && (
                <div className="mt-2 p-3 bg-green-50 rounded-xl flex justify-between text-sm">
                  <span className="text-green-700 font-medium">Cambio a devolver:</span>
                  <span className="text-green-700 font-bold">{formatCurrency(change, 'L')}</span>
                </div>
              )}
          ...
          <div>
            ...
            <textarea className="textarea" rows={2} placeholder="Notas adicionales..."
